...
COPY public.card (number, expiry_month, expiry_year, card_type, code, name, issuer_country, issuer_name_id) FROM stdin;
\.
...
COPY public.currency (id, title, definition) FROM stdin;
2	RUB	russian ruble
3	BUN	\N
\.
...
COPY public.issuer_name (issuer_name, id) FROM stdin;
sberbank	1
t-bank	2
\.
...
COPY public.payment (uuid, status_id, paid, amount, currency_id, created_at, expired_at, description, paymnent_type_id, card_number, recepient_account_number, refundable, test, income) FROM stdin;
\.
...
COPY public.payment_type (id, type) FROM stdin;
1	SBP
2	bank_card
\.
...
COPY public.recepient (account_number, title) FROM stdin;
